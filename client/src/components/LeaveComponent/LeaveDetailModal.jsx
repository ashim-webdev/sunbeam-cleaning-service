...
import { useUpdateLeaveMutation } from "../../redux/slices/api/leaveApiSlice"
...
export function LeaveDetailModal({ request, isAdmin, onClose }) {
  const { LightMode } = useSelector((state) => state.auth);

  const [updateLeaveStatus] = useUpdateLeaveMutation();
  ...
  const handleUpdateStatus = async (data, newStatus) => {
    setIsUpdating(true);
  ...
  return (
    <div className={`
      ...
      fixed inset-0 w-full backdrop-blur-sm flex items-center justify-center overflow-y-auto overflow-x-hidden bg-opacity-50 z-80 transition-colors duration-300 ease-in-out
    `}>
      <AnimatePresence>
        <motion.div 
          ...
          className='absolute py-4 top-0 bottom-0 w-full h-fit px-2.5 flex items-center justify-center'
          >
          <div className={`
            ${LightMode 
              ? "bg-white/90" 
          ...
          `}>
          <div className={`absolute left-0 right-0 top-0 ${LightMode ? "bg-white " : "bg-black/90"} border-b border-gray-300 rounded-t-lg px-6 py-4 flex items-center justify-between transition-colors duration-300 ease-in-out`}>
          ...
          <div className="px-6 pt-28 pb-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {isAdmin && (
                <div className={`flex items-center gap-3 p-3 ${LightMode ? "bg-gray-200" : "bg-white/10"} border border-blue-600/50 rounded-lg transition-colors duration-300 ease-in-out`}>
                  <User className={` ${LightMode ? "text-gray-700" : "text-gray-200"} transition-colors duration-300 ease-in-out`} size={20} />
                  <div>
                    <p className={`${LightMode ? "text-black" : "text-white"} text-xs uppercase tracking-wide transition-colors duration-300 ease-in-out`}>{request.user?.name}</p>
                    <p className={` font-medium ${LightMode ? "text-gray-800" : "text-gray-400"}`}>{request.user?.email}</p>
                  </div>
                </div>
              )}
              ...
              <div className={`flex items-center gap-3 p-3 ${LightMode ? "bg-gray-200" : "bg-white/10"} border border-blue-600/50 rounded-lg `}>
              ...
              {request.status !== 'pending' && (
                <div className={`flex items-center gap-3 p-3 ${LightMode ? "bg-gray-200" : "bg-white/10"} border border-blue-600/50 rounded-lg `}>
                  <Calendar className={` ${LightMode ? "text-gray-700" : "text-gray-200"} transition-colors duration-300 ease-in-out`}size={20} />
                  <div>
                    <p className={`${LightMode ? "text-black" : "text-white"} text-xs uppercase tracking-wide transition-colors duration-300 ease-in-out`}>
            ...
            {isAdmin && request.status === 'pending' && (
              <form>
                <div className={`mt-10 p-3 ${LightMode ? "bg-gray-50 shadow-darkSM" : "bg-black/70 shadow-lightSM"} rounded-lg transition-colors duration-300 ease-in-out`}>
